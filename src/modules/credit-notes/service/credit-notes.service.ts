...
export async function getCreditNotesByOrgSlug(
  ...
  const { data, error } = await supabase
    ...
    .select(
      ...
      sales_orders(sale_number, invoice_number, invoice_type, tipo_factura, total_amount, arca_status, arca_point_of_sale, arca_voucher_number, arca_voucher_type_code, arca_authorized_at)
    `
    )
    .eq("organization_id", org.id)
