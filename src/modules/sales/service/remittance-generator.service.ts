...
export function generateRemittanceHTML(data: RemittanceData): string {
  ...
  const itemsHTML = data.items
    ...
    <tr>
      ...
      <td>${displayValue(item.name)}${item.brand ? ` <span class="brand">${displayValue(item.brand)}</span>` : ""}${showUnitPrice ? (item.extras ?? []).map((extra) => `<div class="extra">+ ${displayValue(extra.description)} · ${formatCurrency(extra.unitPrice, displayCurrency)}/u</div>`).join("") : ""}</td>
