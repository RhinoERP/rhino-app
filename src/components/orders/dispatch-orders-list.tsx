...
async function loadDeclaredValueDefault(
  orgSlug: string,
  totalAmount: number
): Promise<string> {
  const result = await getOrganizationSettings(orgSlug);
  if (!(result.success && result.data)) {
    return "";
  }
  const percentage = result.data.dispatch_declared_value_percentage;
