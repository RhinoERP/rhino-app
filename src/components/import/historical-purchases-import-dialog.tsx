...
import { parseHistoricalPurchasesExcel } from "@/lib/historical-purchases-parser";
...
import { HistoricalPurchasesPreview } from "./historical-purchases-preview";

type HistoricalPurchasesImportDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImport: (data: HistoricalPurchaseRowData[]) => Promise<void>;
};

export function HistoricalPurchasesImportDialog({
  open,
  onOpenChange,
  onImport,
}: HistoricalPurchasesImportDialogProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [parsedData, setParsedData] = useState<
    HistoricalPurchaseRowData[] | null
  >(null);
  ...
  const handleDownloadTemplate = () => {
    downloadTemplate("historical_purchases");
  };
  ...
  const processFile = async (file: File) => {
    ...
    const result = await parseHistoricalPurchasesExcel(file);
  ...
  return (
    <Dialog onOpenChange={handleClose} open={open}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>Importar Compras Históricas</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              Descargar plantilla de compras históricas
            </Button>
          </div>
          ...
          {/* Preview Section */}
          {parsedData && <HistoricalPurchasesPreview data={parsedData} />}
