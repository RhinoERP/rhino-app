...
type BulkSupplierPaymentPreviewProps = {
  preview: BulkSupplierPaymentDistribution[] | undefined;
  isLoading: boolean;
  totalAmount: number;
};
...
function PreviewItem({
  ...
  dist: BulkSupplierPaymentDistribution;
  index: number;
}) {
  return (
    <div className="space-y-1 rounded-md border p-3" key={dist.accountId}>
      <div className="flex items-center justify-between">
        <span className="font-medium text-sm">
          {index + 1}.{" "}
          {dist.purchaseNumber
            ? `Orden de Compra #${dist.purchaseNumber}`
            : "Sin número"}
        </span>
        <span
          className={cn(
  ...
  return (
    <>
      ...
      <div className="space-y-2 rounded-lg bg-muted p-4">
        ...
        {creditBalance > 0 && (
          <div className="flex justify-between text-sm">
            <span>Saldo a Favor (Crédito con Proveedor):</span>
        ...
        <div className="flex justify-between font-semibold">
          <span>Órdenes de Compra Afectadas:</span>
      ...
      {creditBalance > 0 && (
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-3">
          <p className="text-blue-800 text-sm">
            ℹ️ El saldo a favor quedará disponible para ser aplicado en futuras
            compras con este proveedor.
          </p>
        </div>
      )}
    </>
...
export function BulkSupplierPaymentPreview({
  preview,
  isLoading,
  totalAmount,
}: BulkSupplierPaymentPreviewProps) {
  const appliedAmount =
    ...
    return (
      <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
        <p className="text-sm text-yellow-800">
          No hay cuentas pendientes para este proveedor.
        </p>
      </div>
    );
  }
