...
Web Locks evita dos replays simultaneos cuando el navegador lo soporta. `BroadcastChannel` notifica cambios a otras pantallas o pestanas. La idempotencia del servidor sigue siendo la defensa final cuando esas APIs no estan disponibles.
...
Los errores reintentables usan backoff exponencial entre 5 segundos y 5 minutos. `AUTH_REQUIRED` permanece en cola. Conflictos comerciales no se reintentan automaticamente.
